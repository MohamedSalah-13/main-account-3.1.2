...
مستوفيًا القواعد التسع التالية**. لا تفتح ملفًا لتطبيقها وحدها، ولا تتركه دونها.
...
### ق-ل8 — الشاشة الجديدة أو المعدَّلة تخرج ومعها اختبار

«اختبار شاشة» هنا **لا يعني نقرًا آليًّا**: لا يوجد TestFX ولا Monocle في الـ pom، وإضافتهما
قرار قائم بذاته (تبعية جديدة + إعداد headless) لا يُدسّ داخل تعديل شاشة. والمطلوب شقّان،
كلاهما يعمل اليوم:

**١ — منطق الشاشة يخرج إلى صنف مسمّى تحت `features/<area>/` ومعه اختبار وحدة.** هذا هو
ق-ل7 مقروءًا من الجهة الأخرى: المعيار الحاسم واحد — هل يُختبر دون إقلاع JavaFX toolkit؟
ما يبقى في الـ controller هو الربط فقط: قراءة الحقول، نداء الخدمة، عرض النتيجة.

```java
// ❌ داخل الـ controller
if (txtBarcode.getText().isBlank() && txtName.getText().isBlank()) { ... }

// ✅ صنف مسمّى + اختبار
ItemFormValidation.of(form).errors()   // features/items/ItemFormValidation
```

**٢ — الشاشة نفسها مغطاة بالحرّاس البنيويين الأربعة، ولا تُضاف إلى أي قائمة خط أساس.**
لا شيء من هذا يقلع toolkit؛ كلها تقرأ الملفات نصًّا وتعمل في كل بناء:

| الحارس | ما يمسكه في الشاشة |
|---|---|
| `FxmlArchitectureTest` | كل `%key` موجود في الحزم الثلاث، والملف لا يُحمَّل بلا `ResourceBundle` |
| `FxmlWiringArchitectureTest` | `onAction="#method"` يسمّي دالة موجودة، وكل `@FXML` له `fx:id` مطابق |
| `TableColumnArchitectureTest` | لا `PropertyValueFactory` جديد في جدول الشاشة |
| `KeyboardNavigationArchitectureTest` | ترتيب التنقل بـ Enter (ق-ل9) |

**«معدَّلة» تعني معدَّلة لأي سبب.** إصلاح عطب في شاشة قديمة يخرج بالاختبار كما تخرج الشاشة
الجديدة — وإلا بقيت الخمسون شاشة الأخرى بلا تغطية إلى الأبد. وإن كان المنطق المستخرَج سطرين
لا يستحقّان صنفًا، فالواجب المتبقّي هو شطب الشاشة من قوائم خط الأساس التي تظهر فيها.

### ق-ل9 — ترتيب التنقل بين الحقول بـ Enter يُعلَن مرة واحدة

الشاشات هنا تُملأ بالكتابة السريعة وقارئ الباركود في اليد، **وقارئ الباركود ينهي قراءته
بـ Enter**. على شاشة لا تعلن ترتيبًا هذا الـ Enter لا يفعل شيئًا: يمسح المستخدم الكود، يرفع
رأسه، فيجده في الخانة التي كان قد غادرها. ومدّ اليد إلى الفأرة بين كل حقلين هو التكلفة نفسها
مدفوعة ببطء.

الأداة موجودة منذ البداية — `Utils.whenEnterPressed(Control...)` — ومستعملة في **٥ شاشات من
٢٣** فقط:

```java
// ✅ الترتيب يُكتب مرة واحدة، بترتيب ملء النموذج الفعلي، وينتهي بزرّ الحفظ
whenEnterPressed(txtBarcode, txtItemName, txtBuyPrice, txtSelPrice, txtBalance, btnSave);
```

ثلاثة قيود على استعماله يجب معرفتها قبل الاعتماد عليه:

- **الترتيب هو ترتيب الملء لا ترتيب الإعلان.** في شاشة الصنف يبدأ المستخدم بمسح الباركود ثم
  الاسم؛ `AddItemController` اليوم يبدأ بالاسم ثم الباركود، وهو ما لا يستطيع أي اختبار أن
  يقوله — يقوله من رأى الكاشير يعمل.
- **`whenEnterPressed` تستعمل `setOnKeyPressed`، وهو يستبدل أي معالج مفاتيح قائم على الحقل
  ويُستبدَل بأي معالج يُضاف بعده.** فشاشة تريد Enter أن يفعل شيئًا آخر أيضًا (بحث، إضافة سطر)
  تفقد أحدهما صامتة. حتى يتحوّل المساعد إلى `addEventHandler` لا تجمع الاثنين على حقل واحد.
- **ما بعد آخر عنصر لا يُحفَظ تلقائيًّا.** آخر وسيط هو زرّ الحفظ فيصل إليه التركيز؛ الضغط
  عليه يبقى فعل المستخدم.

الحقول التي ليست `TextField` — `ComboBox`، `DatePicker` — تدخل الترتيب نفسه، فالمعامل
`Control`.

**الحارس:** `KeyboardNavigationArchitectureTest` يعدّ الشاشات ذات الحقلين النصيين فأكثر
ويطالبها بالنداء. خط الأساس **١٨ شاشة** بلا ترتيب، والقائمة تفشل في الاتجاهين: ملف جديد
خارجها يُفشل البناء، وملف نُظِّف ولم يُشطب منها يُفشله كذلك. والحكم بالعدّ النصّي فظّ في
الاتجاهين — شاشة تحمل حقولها في كائن نموذج لا تُرى، وحقلا بحث للقراءة فقط يُحسبان — فهو أرضية
تحت الإغفال الواضح لا شهادة بأن ترتيب أي شاشة هو الصحيح.
...
| ق-ل8 | `FxmlWiringArchitectureTest` | `onAction` يسمّي دالة موجودة، وكل `@FXML` له `fx:id` — بخط أساس بالأسماء |
| ق-ل9 | `KeyboardNavigationArchitectureTest` | ٢٣ شاشة إدخال، ١٨ منها بلا ترتيب Enter — **لا تزيد** |
