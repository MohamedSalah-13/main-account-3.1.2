...
(
    ...
    trial_machine     varchar(128)                        null,
    trial_hash        varchar(256)                        null,
    trial_last_check  date                                null,
    trial_fail_count  int                                 null,
...
(
    ...
    constraint users_pk
        unique (user_name),
    constraint users_activity_chk
        check (`user_activity` in (0, 1)),
    constraint users_available_chk
        check (`user_available` in (0, 1))
);
...
create index processes_data_date_idx
    on processes_data (date_insert);

create index processes_data_table_idx
    on processes_data (table_name, table_id);
...
(
    ...
    updated_at    timestamp default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP,
    user_id       int       default 1                 not null,
    constraint stock_transfer_from_fk
        foreign key (stock_from) references stocks (stock_id),
    constraint stock_transfer_to_fk
        foreign key (stock_to) references stocks (stock_id),
    constraint stock_transfer_users_id_fk
        foreign key (user_id) references users (id),
    constraint stock_transfer_not_same_chk
        check (`stock_from` <> `stock_to`)
);

create index stock_transfer_date_idx
...
create index stock_transfer_list_item_idx
    on stock_transfer_list (item_id);
...
(
    ...
    notes         longtext                                 null,
    first_balance decimal(14, 2) default 0.00              not null,
...
(
    ...
    target_ratio1 decimal(6, 2) default 100.00            not null,
    rate_1        decimal(6, 2) default 0.00              not null,
    target_ratio2 decimal(6, 2) default 0.00              not null,
    rate_2        decimal(6, 2) default 0.00              not null,
    target_ratio3 decimal(6, 2) default 0.00              not null,
    rate_3        decimal(6, 2) default 0.00              not null,
...
(
    ...
    amount      decimal(14, 2) default 0.00              not null,
    date_insert datetime       default CURRENT_TIMESTAMP not null,
    updated_at  timestamp      default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP,
    user_id     int            default 1                 not null,
...
create index expenses_details_date_idx
    on expenses_details (date);

create index expenses_details_treasury_idx
    on expenses_details (treasury_id, date);

create table if not exists suppliers_accounts
(
    account_num           bigint auto_increment
        primary key,
    account_code          int                                      not null,
    account_date          date                                     not null,
    purchase              decimal(14, 2) default 0.00              not null,
...
create index suppliers_accounts_date_idx
    on suppliers_accounts (account_date);

create index suppliers_accounts_numberInv_idx
...
(
    ...
    constraint total_buy_users_id_fk
        foreign key (user_id) references users (id),
    constraint total_buy_invoice_type_chk
        check (`invoice_type` in (1, 2))
);

create index total_buy_date_idx
    on total_buy (invoice_date);

create index total_buy_suppliers_sup_code_fk
    on total_buy (sup_code);

create index total_buy_treasury_idx
    on total_buy (treasury_id, invoice_date);
...
(
    ...
    constraint total_buy_re_users_id_fk
        foreign key (user_id) references users (id),
    constraint total_buy_re_invoice_type_chk
        check (`invoice_type` in (1, 2))
);

create index total_buy_re_date_idx
    on total_buy_re (invoice_date);

create index total_buy_re_sup_idx
    on total_buy_re (sup_id);

create index total_buy_re_treasury_idx
    on total_buy_re (treasury_id, invoice_date);
...
(
    ...
    constraint treasury_deposit_expenses_users_id_fk
        foreign key (user_id) references users (id),
    constraint treasury_deposit_expenses_type_chk
        check (`deposit_or_expenses` in (1, 2))
);

create index treasury_deposit_expenses_date_idx
    on treasury_deposit_expenses (date_inter);

create index treasury_deposit_expenses_treasury_idx
    on treasury_deposit_expenses (treasury_id, date_inter);
...
(
    ...
    constraint treasury_transfers_users_id_fk
        foreign key (user_id) references users (id),
    constraint treasury_transfers_not_same_chk
        check (`treasury_from` <> `treasury_to`)
);

create index treasury_transfers_date_idx
...
create index customers_accounts_date_idx
    on customers_accounts (account_date);

create index customers_accounts_numberInv_idx
...
(
    ...
    constraint total_sales_users_id_fk
        foreign key (user_id) references users (id),
    constraint total_sales_invoice_type_chk
        check (`invoice_type` in (1, 2))
);

create index total_sales_date_idx
    on total_sales (invoice_date);

create index total_sales_sup_code_fk
    on total_sales (sup_code);

create index total_sales_treasury_idx
    on total_sales (treasury_id, invoice_date);
...
(
    ...
    constraint total_sales_re_users_id_fk
        foreign key (user_id) references users (id),
    constraint total_sales_re_invoice_type_chk
        check (`invoice_type` in (1, 2))
);

create index total_sales_re_date_idx
    on total_sales_re (invoice_date);

create index total_sales_re_delegate_idx
    on total_sales_re (delegate_id);

create index total_sales_re_sup_idx
    on total_sales_re (sup_id);

create index total_sales_re_treasury_idx
    on total_sales_re (treasury_id, invoice_date);
...
(
    ...
    value_d     decimal(14, 3) default 1.000             not null,
    date_insert datetime       default CURRENT_TIMESTAMP not null,
    updated_at  timestamp      default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP,
    user_id     int            default 1                 not null,
    constraint units_pk
...
(
    ...
    buy_price                decimal(14, 2) default 0.00              not null,
    sel_price1               decimal(14, 2) default 0.00              not null,
    sel_price2               decimal(14, 2) default 0.00              not null,
    sel_price3               decimal(14, 2) default 0.00              not null,
    unit_id                  int                                      not null,
    mini_quantity            decimal(14, 3) default 1.000             not null,
    first_balance            decimal(14, 3) default 0.000             not null,
...
create index items_package_item_idx
    on items_package (item_id);
...
(
    ...
    stock_id      int                          not null,
    first_balance decimal(14, 3) default 0.000 not null,
    constraint items_stock_uk
        unique (item_id, stock_id),
...
(
    ...
    type_value      decimal(14, 3) default 1.000 not null,
...
create index purchase_item_idx
    on purchase (num);
...
create index purchase_re_item_idx
    on purchase_re (item_id);
...
(
    ...
    price            decimal(14, 2)               not null,
    buy_price        decimal(14, 2)               not null,
    total_sel_price  decimal(14, 2) default 0.00  not null,
    total_buy_price  decimal(14, 2) default 0.00  not null,
    total_profit     decimal(14, 2) default 0.00  not null,
    discount         decimal(14, 2) default 0.00  not null,
    type_value       decimal(14, 3) default 1.000 not null,
    expiration_date  date                         null,
    item_has_package tinyint(1)     default 0     not null,
...
(
    ...
    total_profit    decimal(14, 2) default 0.00  not null,
    discount        decimal(14, 2) default 0.00  not null,
    unit_value      decimal(14, 3) default 1.000 not null,
    expiration_date date                         null,
...
create index sales_package_item_idx
    on sales_package (item_id);

create index sales_package_sales_idx
    on sales_package (sales_id);

create index sales_package_unit_idx
...
create index sales_re_item_idx
    on sales_re (item_id);
...
(
    ...
    constraint user_permission_users_id_fk
        foreign key (user_id) references users (id)
            on update cascade on delete cascade,
    constraint user_permission_chk
        check (`check_status` in (0, 1))
);
...
(
    ...
    open_balance        decimal(14, 2) default 0.00 not null,
    close_balance       decimal(14, 2) default 0.00 not null,
    total_sales         decimal(14, 2) default 0.00 not null,
    total_sales_returns decimal(14, 2) default 0.00 not null,
    total_expenses      decimal(14, 2) default 0.00 not null,
    total_deposits      decimal(14, 2) default 0.00 not null,
    total_withdrawals   decimal(14, 2) default 0.00 not null,
    expected_balance    decimal(14, 2) default 0.00 not null,
    difference          decimal(14, 2) default 0.00 not null,
    invoices_count      int            default 0    not null,
    is_open             tinyint(1)     default 1    null,
    notes               text                        null,
    constraint user_shifts_ibfk_1
        foreign key (user_id) references users (id)
            on delete cascade
);
