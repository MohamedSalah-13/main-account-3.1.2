-- =====================================================================
-- V4_2_0_4: Triggers تلقائية لتسجيل حركات المخزون والخزينة
-- =====================================================================
...
BEGIN
    ...
    SELECT stock_id, user_id, invoice_date
    INTO v_stock_id, v_user_id, v_invoice_date
    FROM total_buy
...
BEGIN
    ...
    -- تسجيل حركة وارد (إرجاع من العميل)
    INSERT INTO stock_movements (
        item_id,
        stock_id,
        movement_date,
...
-- =====================================================================
-- 2) Triggers لتحديث رصيد المخزون تلقائياً
-- =====================================================================
...
FOR EACH ROW
BEGIN
    DECLARE v_current_qty DECIMAL(14,3);
    
    -- التحقق من وجود السجل في items_stock
    ...
    IF v_current_qty IS NULL THEN
        ...
        VALUES (NEW.item_id, NEW.stock_id, 0, NEW.quantity_in - NEW.quantity_out);
    ELSE
        -- تحديث الكمية
        UPDATE items_stock
...
FOR EACH ROW
BEGIN
    -- تسجيل المبلغ النقدي المدفوع إلى الخزينة
    IF NEW.paid_up > 0 THEN
...
FOR EACH ROW
BEGIN
    -- تسجيل المبلغ النقدي المدفوع من الخزينة
    IF NEW.paid_up > 0 THEN
        INSERT INTO treasury_movements (
            treasury_id,
            movement_date,
...
FOR EACH ROW
BEGIN
    IF NEW.paid_from_treasury > 0 THEN
        INSERT INTO treasury_movements (
            treasury_id,
            movement_date,
            movement_type,
...
FOR EACH ROW
BEGIN
    IF NEW.paid_to_treasury > 0 THEN
        INSERT INTO treasury_movements (
            treasury_id,
            movement_date,
            movement_type,
...
BEGIN
    -- تحديث رصيد الخزينة
    UPDATE treasury
    SET amount = amount + NEW.amount_in - NEW.amount_out
    WHERE id = NEW.treasury_id;
    
    -- تحديث balance_after في السجل الحالي
    UPDATE treasury_movements
    SET balance_after = (SELECT amount FROM treasury WHERE id = NEW.treasury_id)
    WHERE id = NEW.id;
END$$
...
FOR EACH ROW
BEGIN
    DECLARE v_open_shift_id INT;
    
    -- البحث عن الوردية المفتوحة للمستخدم والخزينة
    ...
    -- ربط الفاتورة بالوردية إذا وجدت
...
-- =====================================================================
-- 6) Trigger لمنع حذف أو تعديل الحركات بعد الترحيل
-- =====================================================================
...
FOR EACH ROW
BEGIN
    -- السماح بالحذف فقط للمستخدم الإداري
