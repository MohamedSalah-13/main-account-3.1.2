تمام — سأعمل لك **سكربت ترحيل (migration) متسلسل وآمن** ينقل قاعدة البيانات الموجودة من بنية `V001` الأصلية إلى البنية المحسّنة **بدون فقدان بيانات**، مع نقاط تحقق (checkpoints) وإمكانية التراجع.

## قبل التشغيل — 3 خطوات إلزامية

1. **خذ Backup كامل**:
```shell script
mysqldump -u root -p --single-transaction --routines --triggers --events account_system_db > backup_before_v018.sql
```

2. **أوقف التطبيق** قبل التشغيل (لتجنب أي كتابة أثناء الـ ALTER).
3. طبّق على **نسخة اختبار أولاً**، وتحقق من الـ Views بعد التشغيل.

السكربت سأضعه في `docs/scripts/updates/V018_migrate_v001_improvements.sql` (هذا هو المكان المخصص للتحديثات).
...
1. خذ Backup (راجع الأعلى).
2. نفّذ ملف الترحيل:
```shell script
mysql -u root -p --default-character-set=utf8mb4 account_system_db < docs/scripts/updates/V018_migrate_v001_improvements.sql
```

3. أعد إنشاء الـ Views من النسخة الجديدة:
```shell script
mysql -u root -p --default-character-set=utf8mb4 account_system_db < docs/scripts/main/V4_1_0_7_view_table.sql
```

4. تحقّق من السلامة:
```sql
SHOW ENGINE INNODB STATUS;     -- لا أخطاء
   SELECT COUNT(*) FROM total_buy;     -- نفس العدد قبل وبعد
...
## للتراجع (rollback)
لا يوجد تراجع تلقائي آمن بعد تحويل الأنواع؛ **الاعتماد على الـ Backup**:
```shell script
mysql -u root -p account_system_db < backup_before_v018.sql
```
...
هل تريد:
1. **أن أفحص الـ Triggers (V002–V014)** وأحدّث أي نوع `INT` إلى `BIGINT` للتوافق مع الـ `invoice_number` الجديد؟
2. **أو أرسل لك مسح للأماكن في Java** التي تقرأ `getInt` للحقول التي أصبحت `BIGINT`؟
3. **أو ننتقل مباشرة لتصميم السنة المالية** كما اتفقنا؟