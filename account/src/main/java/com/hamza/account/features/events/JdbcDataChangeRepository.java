...
/** JDBC access to {@code data_change}: one monotonically-versioned row per topic. */
public final class JdbcDataChangeRepository extends AbstractDao<Object> {

    /**
     * {@code NOW(6)} is written explicitly rather than left to {@code ON UPDATE
    ...
    private static final String ANNOUNCE = """
            INSERT INTO data_change (topic, revision, changed_at, changed_by)
            VALUES (?, 1, NOW(6), ?)
    ...
    private static final String READ_ALL =
            "SELECT topic, revision, changed_at, changed_by FROM data_change";

    /** What each topic looked like at one moment: its version, time, and source machine. */
    public record Change(long revision, Timestamp changedAt, String changedBy) {
    }
            ...
            try (PreparedStatement statement = connection.prepareStatement(READ_ALL);
                ...
                while (resultSet.next()) {
                    changes.put(resultSet.getString("topic"),
                            new Change(resultSet.getLong("revision"),
