...
public final class OfferForm {
     ...
     * the group's for a quantity offer), the quantity bought, the quantity given and its discount, and the two
     * limits, any of them blank.
     */
    ...
    public static Offer build(int id, String name, OfferKind kind, OfferStatus status, LocalDate startsOn,
        ...
        Offer offer = new Offer(id, name == null ? "" : name.strip(), kind,
                ...
                kind == OfferKind.PERCENT ? value : null,
                kind == OfferKind.AMOUNT ? value : null,
                kind == OfferKind.PRICE || kind == OfferKind.QUANTITY_PRICE ? value : null,
                kind == OfferKind.PERCENT ? null : unitId,
                pooled ? given.buyQuantity() : null,
                kind == OfferKind.BUY_GET ? given.getQuantity() : null,
                kind == OfferKind.BUY_GET ? given.getPercent() : null,
                given.maxPerInvoice(), given.quantityLimit(),
    ...
    public static void requireValid(Offer offer) throws UserValidationException {
        ...
        if (offer.targets().stream().noneMatch(target -> !target.excluded() && !target.reward())) {
    ...
    private static void requireOnlyWhatTheKindUses(Offer offer) throws UserValidationException {
        ...
        boolean stray = (kind != OfferKind.PERCENT && offer.percent() != null)
                || (kind != OfferKind.AMOUNT && offer.amount() != null)
                || (kind != OfferKind.PRICE && kind != OfferKind.QUANTITY_PRICE && offer.offerPrice() != null)
                || (!kind.pooled() && offer.buyQuantity() != null)
     ...
     * Once a line names the offer, those are all that may change (ق-ع٧): stop it and write another. The limits
     * are terms too: an edit of a saved invoice is judged by them.
     */
