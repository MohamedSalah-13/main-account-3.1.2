...
--
-- Flyway re-runs this file whenever its checksum changes, so a client on
-- any older schema ends up with the current set of views without needing a
-- hand-run script. This is what makes the missing view_customer_receivables
-- class of failure impossible to repeat.
-- 
-- DROP VIEW IF EXISTS + CREATE VIEW is used instead of CREATE OR REPLACE
-- VIEW: the latter fails if the name is occupied by a base table, and it
-- cannot change a view's column count.
-- =====================================================================

-- #####################################################################
-- ## Reporting views                          (was V008_view_table) ##
-- #####################################################################
-- =====================================================================
...
CREATE VIEW purchase_names_table AS
SELECT p.id,
       p.invoice_number,
       p.num,
       p.type,
...
CREATE VIEW sales_names_table AS
SELECT s.id,
       s.invoice_number,
       s.num,
       s.type,
...
CREATE VIEW sales_return_names_table AS
SELECT sr.id,
       sr.invoice_number,
       sr.item_id,
       sr.source_line_id,
...
CREATE VIEW purchase_return_names_table AS
SELECT pr.id,
       pr.invoice_number,
       pr.item_id,
       pr.source_line_id,
...
CREATE VIEW stock_transfer_view AS
SELECT st.id,
       st.transfer_date,
       st.stock_from,
       st.stock_to,
...
CREATE VIEW quantity_items_table AS
...
CREATE VIEW total_purchase_names_table AS
...
CREATE VIEW card_item_view_details AS
SELECT c.id,
       c.invoice_number,
       c.invoice_date,
       c.item_num,
...
CREATE VIEW expenses_details_view AS
SELECT ed.id,
       ed.type_code,
       ed.date,
       ed.amount,
...
CREATE VIEW target_delegate AS
...
CREATE VIEW treasury_transfers_and_names AS
SELECT tt.id,
       tt.treasury_from,
       tt.treasury_to,
       tt.amount,
...
CREATE VIEW account_customer_totals AS
...
CREATE VIEW account_suppliers_totals AS
