...
-- V1 - Baseline schema.
--
-- This is the schema as shipped to clients in v4.1.3, assembled verbatim from
-- the manual bundle they ran (V001_tables, V002_audit_log, V003_trigger_company,
-- V004_trigger_items, V006_user_insert, V007_user_truncate, V008_view_table).
--
-- It is the Flyway baseline: existing client databases are stamped with it
-- WITHOUT executing it (flyway.baselineOnMigrate / baselineVersion=1), because
-- they already contain exactly this. A brand-new database executes it in full
-- and then continues with V2, V3, ... - which is why every later migration must
-- be a separate file and is NEVER folded back into this one.
--
-- Two deliberate changes from the original scripts:
...
-- #####################################################################
-- ## write_audit_log procedure + items audit triggers (was V002)      ##
-- #####################################################################
...
-- #####################################################################
-- ## Company triggers and seed                (was V003_trigger_company) ##
-- #####################################################################
-- user permission
...
-- #####################################################################
-- ## Items trigger + max_item_id procedure      (was V004_trigger_items) ##
-- #####################################################################
-- items
